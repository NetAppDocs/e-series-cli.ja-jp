...
summary: blockedEventAlertList コマンドは、blockEventAlert コマンドによって現在ブロックされているイベントのリストを返します。このリストのイベントは、アラート端末とパラメータを使用して設定した通知、つまり電子メール、syslog、およびトラップ通知を送信しないイベントです。 
---
