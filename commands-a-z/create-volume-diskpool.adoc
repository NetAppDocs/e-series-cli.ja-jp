...
NOTE: 標準ボリューム名とシンボリューム名の文字数制限は60文字です。ボリューム名には、英数字、アンダースコア（_）、ハイフン（-）、およびポンド記号（#）を任意の組み合わせで使用できます。
...
各ボリューム名は一意である必要があります。ユーザーラベルには、英数字、アンダースコア（_）、ハイフン（-）、ハッシュ記号（#）を任意の組み合わせで使用できます。標準ボリューム名とシンボリューム名は、最大60文字まで使用できます。
